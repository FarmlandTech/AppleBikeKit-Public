...
static FL_MAINBATT_INST_T mBatt_inst;
...
static int DUFRequestHandler(uint8_t* data, uint32_t leng)
{
	...
	case OPC_WRITE_TO_CACHE:
	{
		...
		{
			mBatt_inst.cache_mem[x] = data[data_index];
			data_index++;
		}
	...
	case OPC_PROGRAM_FLASH:
	{
		...
		{
			vm_flash_mem[prog_start_index] = mBatt_inst.cache_mem[cache_index];
			cache_index++;
			prog_start_index++;
		}
	...
	ISOTP_SenderDelegate(mBatt_inst.isotp_inst,
...
static int HostConfigRTCHandler(uint8_t* data, uint32_t leng)
{
	...
	COPY_MIN_ARRAY(data, leng, &info.bytes[0], sizeof(info.bytes));
	mBatt_inst.info.unix_time = info.bits.unix_time;
	return RESPONSE_SUCCESS;
}
...
static int HostResetHandler(uint8_t* data, uint32_t leng)
{
	...
	if (requst.bits.device_type == DEVICE_OBJ_HMI)
	{
		...
		if (mBatt_inst.send_handler)
		{
			mBatt_inst.send_handler(FL_CANID_HOST_DEVICE_RESET_RSP, false, response.bytes, sizeof(response.bytes));
		}
	}
...
void __stdcall FL_device_MainBatt_add_warning(uint8_t code)
{
	...
	{
		if (mBatt_inst.warning_code[search_index] == code)
		{
			break;
		}

		if (mBatt_inst.warning_code[search_index] == 0)
		{
			mBatt_inst.warning_code[search_index] = code;
			mBatt_inst.warning_leng++;
			break;
		}
	}
}

void __stdcall FL_device_MainBatt_remove_warning(uint8_t code)
{
	uint8_t search_index = 0;
	...
	for (search_index = 0; search_index < WARNING_CODE_SIZE; search_index++)
	{
		if (mBatt_inst.warning_code[search_index] == code)
		{
			mBatt_inst.warning_code[search_index] = 0;
			for (; search_index < (WARNING_CODE_SIZE -1); search_index++)
			{
				if (mBatt_inst.warning_code[(search_index+1)] != 0)
				{
					mBatt_inst.warning_code[search_index] = mBatt_inst.warning_code[(search_index + 1)];
					mBatt_inst.warning_code[(search_index + 1)] = 0;
				}
				else
				{
					break;
...
void __stdcall FL_device_MainBatt_add_error(uint8_t code)
{
	uint8_t search_index = 0;
	...
	{
		if (mBatt_inst.error_code[search_index] == code)
		{
			break;
		}

		if (mBatt_inst.error_code[search_index] == 0)
		{
			mBatt_inst.error_code[search_index] = code;
			mBatt_inst.error_leng++;
			break;
		}
	}
}

void __stdcall FL_device_MainBatt_remove_error(uint8_t code)
{
	uint8_t search_index = 0;
	...
	for (search_index = 0; search_index < ERROR_CODE_SIZE; search_index++)
	{
		if (mBatt_inst.error_code[search_index] == code)
		{
			mBatt_inst.error_code[search_index] = 0;
			for (; search_index < (ERROR_CODE_SIZE - 1); search_index++)
			{
				if (mBatt_inst.error_code[(search_index + 1)] != 0)
				{
					mBatt_inst.error_code[search_index] = mBatt_inst.error_code[(search_index + 1)];
					mBatt_inst.error_code[(search_index + 1)] = 0;
				}
				else
				{
					break;
...
static void MainBatt_INFO_00_broadcast_packet(void)
{
	MAIN_BAT_INFO00_T info = { 0 };

	info.bits.charge_fet = mBatt_inst.info.charge_fet_on;
	info.bits.charging = mBatt_inst.info.charging;
	info.bits.fully_charged = mBatt_inst.info.fully_charged;
	info.bits.charge_detected = mBatt_inst.info.charger_detected;
	info.bits.discharge_fet = mBatt_inst.info.charger_detected;
	info.bits.discharging = mBatt_inst.info.discharging;
	info.bits.nearly_discharged = mBatt_inst.info.nearly_discharged;
	info.bits.fully_discharged = mBatt_inst.info.fully_discharged;
	info.bits.design_volt = mBatt_inst.info.design_voltage;
	info.bits.design_capacity = mBatt_inst.info.design_capacity;
	info.bits.cycle_count = mBatt_inst.info.cycle_count;
	info.bits.uncharged_day = mBatt_inst.info.uncharged_day;
	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_INFO_00, false, info.bytes, sizeof(info.bytes));
	}
}

static void MainBatt_INFO_01_broadcast_packet(void)
{
	MAIN_BAT_INFO01_T info = { 0 };

	info.bits.actual_volt = mBatt_inst.info.actual_voltage;
	info.bits.actual_current = mBatt_inst.info.actual_current;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_INFO_01, false, info.bytes, sizeof(info.bytes));
	}
}

static void MainBatt_INFO_02_broadcast_packet(void)
{
	MAIN_BAT_INFO02_T info = { 0 };

	info.bits.temperature = mBatt_inst.info.temperature;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_INFO_02, false, info.bytes, sizeof(info.bytes));
	}
}

static void MainBatt_INFO_03_broadcast_packet(void)
{
	MAIN_BAT_INFO03_T info = { 0 };

	info.bits.rsoc = mBatt_inst.info.rsoc;
	info.bits.asoc = mBatt_inst.info.asoc;
	info.bits.rsoh = mBatt_inst.info.rsoh;
	info.bits.asoh = mBatt_inst.info.asoh;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_INFO_03, false, info.bytes, sizeof(info.bytes));
	}
}

static void MainBatt_RTC_broadcast_packet(void)
{
	MAIN_BAT_RTCINFO_T info = { 0 };

	info.bits.unix_time = mBatt_inst.info.unix_time;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_RTC_INFO, false, info.bytes, sizeof(info.bytes));
	}
}


static void MainBatt_warning_broadcast_packet(void)
{
	...
	warning.bits.total_leng = mBatt_inst.warning_leng;

	while (start_index < end_index)
	{
		if (mBatt_inst.warning_code[start_index] == 0)
		{
			break;
		}
		...
		{
		case 0:
			warning.bits.warning_0 = mBatt_inst.warning_code[start_index];
			break;

		case 1:
			warning.bits.warning_1 = mBatt_inst.warning_code[start_index];
			break;

		case 2:
			warning.bits.warning_2 = mBatt_inst.warning_code[start_index];
			break;

		case 3:
			warning.bits.warning_3 = mBatt_inst.warning_code[start_index];
			break;

		case 4:
			warning.bits.warning_4 = mBatt_inst.warning_code[start_index];
			break;

		case 5:
			warning.bits.warning_5 = mBatt_inst.warning_code[start_index];
			break;

		case 6:
			warning.bits.warning_6 = mBatt_inst.warning_code[start_index];
			break;
		}
	...
	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_HMI_WARNING, false, warning.bytes, sizeof(warning.bytes));
	}
...
static void MainBatt_error_broadcast_packet(void)
{
	...
	error.bits.total_leng = mBatt_inst.error_leng;

	while (start_index < end_index)
	{
		if (mBatt_inst.error_code[start_index] == 0)
		{
			break;
		}
		...
		{
		case 0:
			error.bits.error_0 = mBatt_inst.error_code[start_index];
			break;

		case 1:
			error.bits.error_1 = mBatt_inst.error_code[start_index];
			break;

		case 2:
			error.bits.error_2 = mBatt_inst.error_code[start_index];
			break;

		case 3:
			error.bits.error_3 = mBatt_inst.error_code[start_index];
			break;

		case 4:
			error.bits.error_4 = mBatt_inst.error_code[start_index];
			break;

		case 5:
			error.bits.error_5 = mBatt_inst.error_code[start_index];
			break;

		case 6:
			error.bits.error_6 = mBatt_inst.error_code[start_index];
			break;
		}
	...
	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_HMI_ERROR, false, error.bytes, sizeof(error.bytes));
	}

}


static void MainBatt_debuginfo_00_broadcast_packet(void)
{
	BAT_DEBUGINFO00_T debug_info = { 0 };

	debug_info.bits.cell_1_volt = mBatt_inst.info.cell_1_volt;
	debug_info.bits.cell_2_volt = mBatt_inst.info.cell_2_volt;
	debug_info.bits.cell_3_volt = mBatt_inst.info.cell_3_volt;
	debug_info.bits.cell_4_volt = mBatt_inst.info.cell_4_volt;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_DEBUGINFO_00, true, debug_info.bytes, sizeof(debug_info.bytes));
	}
}


static void MainBatt_debuginfo_01_broadcast_packet(void)
{
	BAT_DEBUGINFO01_T debug_info = { 0 };

	debug_info.bits.cell_5_volt = mBatt_inst.info.cell_5_volt;
	debug_info.bits.cell_6_volt = mBatt_inst.info.cell_6_volt;
	debug_info.bits.cell_7_volt = mBatt_inst.info.cell_7_volt;
	debug_info.bits.cell_8_volt = mBatt_inst.info.cell_8_volt;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_DEBUGINFO_01, true, debug_info.bytes, sizeof(debug_info.bytes));
	}
}

static void MainBatt_debuginfo_02_broadcast_packet(void)
{
	BAT_DEBUGINFO02_T debug_info = { 0 };

	debug_info.bits.cell_9_volt = mBatt_inst.info.cell_9_volt;
	debug_info.bits.cell_10_volt = mBatt_inst.info.cell_10_volt;
	debug_info.bits.cell_11_volt = mBatt_inst.info.cell_11_volt;
	debug_info.bits.cell_12_volt = mBatt_inst.info.cell_12_volt;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_DEBUGINFO_02, true, debug_info.bytes, sizeof(debug_info.bytes));
	}
}


static void MainBatt_debuginfo_03_broadcast_packet(void)
{
	BAT_DEBUGINFO03_T debug_info = { 0 };

	debug_info.bits.cell_13_volt = mBatt_inst.info.cell_13_volt;
	debug_info.bits.cell_14_volt = mBatt_inst.info.cell_14_volt;
	debug_info.bits.cell_15_volt = mBatt_inst.info.cell_15_volt;
	debug_info.bits.cell_16_volt = mBatt_inst.info.cell_16_volt;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_DEBUGINFO_03, true, debug_info.bytes, sizeof(debug_info.bytes));
	}
}

static void MainBatt_debuginfo_04_broadcast_packet(void)
{
	BAT_DEBUGINFO04_T debug_info = { 0 };

	debug_info.bits.cell_17_volt = mBatt_inst.info.cell_17_volt;
	debug_info.bits.cell_18_volt = mBatt_inst.info.cell_18_volt;
	debug_info.bits.cell_19_volt = mBatt_inst.info.cell_19_volt;
	debug_info.bits.cell_20_volt = mBatt_inst.info.cell_20_volt;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_DEBUGINFO_04, true, debug_info.bytes, sizeof(debug_info.bytes));
	}
}

static void MainBatt_debuginfo_05_broadcast_packet(void)
{
	BAT_DEBUGINFO05_T debug_info = { 0 };

	debug_info.bits.temperature_1 = mBatt_inst.info.sensor_temperature_1;
	debug_info.bits.temperature_2 = mBatt_inst.info.sensor_temperature_2;
	debug_info.bits.temperature_3 = mBatt_inst.info.sensor_temperature_3;
	debug_info.bits.temperature_4 = mBatt_inst.info.sensor_temperature_4;
	debug_info.bits.temperature_5 = mBatt_inst.info.sensor_temperature_5;
	debug_info.bits.temperature_6 = mBatt_inst.info.sensor_temperature_6;
	debug_info.bits.temperature_7 = mBatt_inst.info.sensor_temperature_7;
	debug_info.bits.temperature_8 = mBatt_inst.info.sensor_temperature_8;

	if (mBatt_inst.send_handler)
	{
		mBatt_inst.send_handler(FL_CANID_MBAT_DEBUGINFO_05, true, debug_info.bytes, sizeof(debug_info.bytes));
	}
}
...
{
	MainBatt_INFO_00_broadcast_packet();
	MainBatt_INFO_01_broadcast_packet();
	MainBatt_INFO_02_broadcast_packet();
}

static void HMI_broadcast_per_200ms(void)
{
	MainBatt_warning_broadcast_packet();
	MainBatt_error_broadcast_packet();
}

static void HMI_broadcast_per_1000ms(void)
{
	MainBatt_debuginfo_00_broadcast_packet();
	MainBatt_debuginfo_01_broadcast_packet();
	MainBatt_debuginfo_02_broadcast_packet();
	MainBatt_debuginfo_03_broadcast_packet();
	MainBatt_debuginfo_04_broadcast_packet();
	MainBatt_debuginfo_05_broadcast_packet();
}

static void HMI_broadcast_per_3000ms(void)
{
	MainBatt_INFO_03_broadcast_packet();
}
...
uint32_t FL_device_MainBatt_Init(ISOTP_INST_T * isotp_inst, canbus_send_handler_p sender)
{
	if (isotp_inst && sender)
	{
		mBatt_inst.isotp_inst = isotp_inst;
		mBatt_inst.isotp_inst->send_packet = sender;
		mBatt_inst.send_handler = sender;

		ISOTP_Init(mBatt_inst.isotp_inst);

		ISOTP_RegisterListenPort(mBatt_inst.isotp_inst,
			&ISOTP_Port_DeviceDFUSend,
			&ISOTP_Port_DeviceDFURecive);
...
void __stdcall FL_device_MainBatt_update_info(FL_MAINBATT_INFO_T* new_info)
{
	memcpy(&mBatt_inst.info, new_info, sizeof(mBatt_inst.info));
}

uint32_t __stdcall Virtual_Device_MainBatt_Startup(void)
{
...
uint32_t __stdcall Virtual_Device_MainBatt_Stop(void)
{
